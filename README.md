...
Projeto Java 21, com CLI e interface gráfica, baseado no **Trabalho SO.pdf** e nas regras de clareza do documento **Markdown.md colado**. Implementa clock lógico, eventos discretos, processos/threads, FCFS, Round Robin, prioridades não preemptivas, paginação FIFO, E/S e arquivos hierárquicos.
...
Fontes de requisitos: `Trabalho SO.pdf`, Prof. Maurício Acconcia Dias; regras de código: `Markdown.md colado`, fornecidos neste chat. Algoritmos implementados a partir das definições do enunciado; não há biblioteca de núcleo de SO.

**Ferramenta de apoio:** código, documentação e relatório foram produzidos com assistência do ChatGPT/Codex. Essa assistência deve ser declarada conforme as regras da disciplina. O grupo deve revisar, compreender, adaptar quando necessário e conseguir explicar a solução. Não foi atribuído um nome de integrante ou autoria humana fictícia.

A criação/publicação do repositório GitHub não foi realizada. O pacote está pronto para ser colocado em um repositório do grupo. Não há histórico de evolução ou participação fabricado. `docs/progresso.md` é um registro técnico desta implementação, não uma entrega intermediária retroativa. Antes da entrega, registre a participação real, confirme o acesso do professor e repita os comandos de compilação, teste e execução em seu ambiente.
